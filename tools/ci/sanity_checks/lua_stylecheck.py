...
class LuaStyleContext:
    ...
    def preprocess_line(self, line):
        ...
        self.line_no_comments_or_strings = re.sub(
            r"(\"([^\"\\]|\\.)*\"|\'([^\'\\]|\\.)*\')", "strVal", self.line_no_comments
        )
    ...
    def error(self, error_string, suppress_line_ref=False):
        ...
        if self.show_errors:
            ...
            if (
                not suppress_line_ref
                and self.counter > 0
                and self.counter <= len(self.lines)
            ):
                print(
...
class LuaStyleCheck:
    ...
    def run_style_check(self):
        ...
        with open(self.context.filename, "r") as f:
                ...
                if self.context.in_condition:
                    ...
                    if re.search(r"\bthen\b", code_line_no_comments):
                        ...
                        if conditional_visitor:
                            conditional_visitor.visit(
                                self.context.full_condition, self.context
                            )
            ...
            if (
                "DefaultActions" not in self.context.filename
                and uses_id == True
                and not has_id_ref
            ):
