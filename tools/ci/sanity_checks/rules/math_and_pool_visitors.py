...
class MathRandomBoundsVisitor(LuaStyleVisitor):
    def visit(self, line, context):
        ...
        if randString:
            paramList = randString.group(1).split(",")

            # TODO: Support 2 parameter input for floats, so that we can disallow empty parameter calls
            if paramList != [""] and len(paramList) != 2:
                context.error(
